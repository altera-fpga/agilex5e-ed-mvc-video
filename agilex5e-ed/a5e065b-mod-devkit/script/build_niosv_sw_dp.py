...
software_path = script_path.joinpath('../src/sw/dp_subsys')
software_path_vds = script_path.joinpath('src/sw/dp_subsys')
...
HEX_NAME="dp_core_onchip_mem_onchip_mem.hex"

# Application Source Directory
APP_DIR = software_path.joinpath("dp_demo")
...
SW_ROOTDIR = Path(os.getenv("QUARTUS_ROOTDIR")).joinpath("../ip/altera/altera_dp/software")

# Location containing the working copy for the system libraries files
LIB_BASE = software_path

# Names of the bitex libraries
RX_LIB_NAME = "btc_dprx_syslib"
TX_LIB_NAME = "btc_dptx_syslib"
TXLL_LIB_NAME = "btc_dptxll_syslib"

# Location containing all the application source files
APP_SRC_DIR = SW_ROOTDIR.joinpath("dp_demo_src")

# Location containing the original system libraries files
RX_LIB_SRC_DIR = SW_ROOTDIR.joinpath(RX_LIB_NAME)
TX_LIB_SRC_DIR = SW_ROOTDIR.joinpath(TX_LIB_NAME)
TXLL_LIB_SRC_DIR = SW_ROOTDIR.joinpath(TXLL_LIB_NAME)

# BSP Directory
BSP_DIR = software_path.joinpath("dp_demo_bsp")
BSP_DIR_VDS = software_path_vds.joinpath("dp_demo_bsp")
...
BSP_SETTINGS.append(["hal.enable_c_plus_plus", "0"])
BSP_SETTINGS.append(["hal.enable_clean_exit", "0"])
BSP_SETTINGS.append(["hal.enable_exit", "0"])
BSP_SETTINGS.append(["hal.enable_lightweight_device_driver_api", "0"])
BSP_SETTINGS.append(["hal.enable_reduced_device_drivers", "0"])
BSP_SETTINGS.append(["hal.enable_runtime_stack_checking", "0"])
BSP_SETTINGS.append(["hal.linker.use_picolibc", "1"])
BSP_SETTINGS.append(["hal.enable_sim_optimize", SIMULATION_OPTIMIZED_SUPPORT])
BSP_SETTINGS.append(["hal.make.cflags_optimization", OPTIMIZATION_LEVEL])
BSP_SETTINGS.append(["hal.linker.allow_code_at_reset", "1"])
BSP_SETTINGS.append(["hal.linker.enable_alt_load", "1"])
BSP_SETTINGS.append(["hal.linker.enable_alt_load_copy_exceptions", "0"])
BSP_SETTINGS.append(["hal.linker.enable_alt_load_copy_rodata", "0"])
BSP_SETTINGS.append(["hal.linker.enable_alt_load_copy_rwdata", "1"])
BSP_SETTINGS.append(["hal.linker.enable_exception_stack", "0"])
BSP_SETTINGS.append(["hal.linker.exception_stack_memory_region_name", SOPC_CODE_MEMORY_NAME])
BSP_SETTINGS.append(["hal.max_file_descriptors", "4"])
...
BSP_SETTINGS.append(["hal.sys_clk_timer", "none"])
BSP_SETTINGS.append(["hal.timestamp_timer", SOPC_TIMER_NAME])

QPF_PATH = script_path.joinpath('../', "top.qpf")
# VDS_PATH is relative to where the .QSF file is located
VDS_PATH = script_path.joinpath('src/vds/dp_core', "dp_core.vds")
###############################################################################################################
# Functions
...
def copy_hdcp_source():
    # ::WARNING:: If you used spaces in your ACDS install location
    # this script will not work. You will need to copy the software
    # directory to another location and update this script appropriately
    HDCP_ROOTDIR = Path(os.getenv("QUARTUS_ROOTDIR")).joinpath("../ip/altera/hdcp2x/software")

    # Location containing the original system libraries files
    HDCP_LIB_SRC_DIR=HDCP_ROOTDIR.joinpath("dp")
    copy_directory(HDCP_LIB_SRC_DIR, APP_DIR)

# Main function
def build_software():
    # Copy the libraries across
    copy_directory(RX_LIB_SRC_DIR, LIB_BASE.joinpath(RX_LIB_NAME))
    copy_directory(TX_LIB_SRC_DIR, LIB_BASE.joinpath(TX_LIB_NAME))
    copy_directory(TXLL_LIB_SRC_DIR, LIB_BASE.joinpath(TXLL_LIB_NAME))
    
    # Select the required libraries
    RX_LIB_DIR = LIB_BASE.joinpath( RX_LIB_NAME, "release_mst", RX_LIB_NAME )
    TX_LIB_DIR = LIB_BASE.joinpath( TX_LIB_NAME, "release_mst", TX_LIB_NAME )
    TXLL_LIB_DIR = LIB_BASE.joinpath( TXLL_LIB_NAME, "release", TXLL_LIB_NAME )
    ...
    run_shell_cmd( 'niosv-app --bsp-dir={0} --app-dir={1} --srcs={2} --link-lib={3} --link-lib={4} --link-lib={5} --elf-name={6}'.
                  format(BSP_DIR, APP_DIR, APP_DIR, RX_LIB_DIR, TX_LIB_DIR, TXLL_LIB_DIR, ELF_NAME) )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build DisplayPort Subsystem Software")
